...
public static class Wallet
{
    ...
    public float getBill(ArrayList<Food> addtocart,ArrayList<Integer> qty,int restno,ArrayList<Restaurant> RList,ArrayList<Customer> CList,int number,Company C,Cart c)
    {
      float amount1=0;
...
public static class Cart
{
    ...
    public void setflag(int a){flag=a;}
    public void add(Food food,int qty,int rest){addtocart.add(food);        //food is being added to the cart of the person
      resqty.add(qty);
      restno=rest;}
      public void checkout(ArrayList<Customer> CList,ArrayList<Restaurant> RList,int number,Company C,Cart c,ArrayList<String> OrderHistory)
      {
          flag=0;
            ...
            if(CList.get(number).getamount()+CList.get(number).getpoint()>=billamount)
              {
                  ...
                  for(int ci=0;ci<addtocart.size();ci++){System.out.println(Integer.toString(resqty.get(ci))+" "+RList.get(restno).getname()+" : "+addtocart.get(ci));}
                ...
                if(checkpermission==1){
                    ...
                    if(CList.get(number).getpoint()<billamount){
                      ...
                      for(int n=0;n<addtocart.size();n++){
                        OrderHistory.add("Bought item: "+addtocart.get(n).getname()+" , quantity: "+Integer.toString(resqty.get(n))+" for Rs "+Float.toString(addtocart.get(n).getPrice()*resqty.get(n)*(1-addtocart.get(n).getDiscount()/100))+" from "+RList.get(restno).getname());
                      }
                  ...
                  if(ccc==1)
                  {
                      for(int k=0;k<addtocart.size();k++){System.out.println(addtocart.get(k));}
                      System.out.println("Enter food name to be removed");
                      in.nextLine();
                      String removeid=in.nextLine();
                      for(int h=0;h<addtocart.size();h++){
                        if(addtocart.get(h).getname().equals(removeid)){System.out.println("Removed");addtocart.remove(h);resqty.remove(h);}}     //item removed
...
public static class Customer extends Wallet
{
  ...
  public void Select(Food food,int qtyneeded,int restno)
  {         //restno is Restaurant number
  ...
  public float getDelivery(float amount){return (float)40;}     //rs 40 per delivery for normal
  public float finalcheck(float amount){return amount;}         //special offers on more discount
  ...
  {
    for(int k=0;k<OrderHistory.size();k++){System.out.println(OrderHistory.get(k));}
  }
...
{
  Elite(String name){super(name);}
  @Override
    public float getDelivery(float amount){return (float)0.0;}    //free delivery
  @Override
    public float finalcheck(float amount){if(amount>200){return amount-50;}return amount;}
  @Override
    public String getCategory(){return "(Elite)";}
}
...
public static class Special extends Customer
{
  ...
  @Override
    public float getDelivery(float amount){return (float)20;}     //rs 20 per delivery
  @Override
    public float finalcheck(float amount){if(amount>200){return amount-25;}return amount;}
  @Override
    public String getCategory(){return "(Special)";}
}
...
public static class Food
{
  ...
  {
    return Integer.toString(ID)+" "+name+" "+Float.toString(price)+" "+Integer.toString(qty)+" "+Float.toString(discount)+"% off "+category;
  }
  public void setname(String S){name=S;}
  public void setprice(float p){price=p;}
  public void setqty(int q){qty=q;}
  public void setdisc(float disco){discount=disco;}
  public void setcate(String S){category=S;}
}
...
public static class Restaurant extends Wallet
{
  ...
  public void setnorder(int a){norder+=a;}
  public int getnorder(){return norder;}
  public ArrayList<Food> getFoodList(){return FList;}
  public String getname(){return name;}
  public void setname(String S){name=S;}
  public void Addfood()
  {
    ...
    offer=in.nextLine();
    Food food=new Food(FList.size()+1,Float.parseFloat(offer),Float.parseFloat(price),name,category,qty);
  ...
  public void Editfood()
  {
    ...
    for(int i=0;i<FList.size();i++){System.out.println(FList.get(i));}
    ...
    in.nextLine();
    if(attchoice==1){System.out.println("Enter the new Name:");
    String str=in.nextLine();
    FList.get(foodch-1).setname(str);}
    else if(attchoice==2){System.out.println("Enter the new price:");
    //in.nextLine();
    String prc=in.nextLine();
    FList.get(foodch-1).setprice(Float.parseFloat(prc));}
    else if(attchoice==3){System.out.println("Enter the new Quantity:");
    int qq=in.nextInt();
    FList.get(foodch-1).setqty(qq);}
    else if(attchoice==4){System.out.println("Enter the new Category:");
    String cat=in.nextLine();
    FList.get(foodch-1).setcate(cat);}
    else{System.out.println("Enter the new Offer:");
    String off=in.nextLine();
    FList.get(foodch-1).setdisc(Float.parseFloat(off));}
    System.out.println(FList.get(foodch-1));
  }
  public void PrintReward(){System.out.println("Total rewards: "+Float.toString(getpoint()));}
  ...
  public float getprdis(){return personaldisc;}
  public float Discount(float amount){return amount;}     //the Restaurant discount for normal rests is zero
  public float REWARD(float spentamt)
  {
    float point=0;
    while(spentamt>=100){point+=5;spentamt-=100;}return point;}
}

public static class FastFood extends Restaurant
{
  FastFood(String name){super(name);}
  @Override
  public float Discount(float amount){return amount-(getprdis()/100);}
  @Override
  public float REWARD(float spentamt){float point=0;
    while(spentamt>=150){point+=10;spentamt-=150;}return point;}
}

public static class Authentic extends Restaurant
{
  Authentic(String name){super(name);}
  @Override
  public float Discount(float amount){amount-=amount*(getprdis()/100);
    if(amount>100)return amount-50;
    else return amount;}
    @Override
    public float REWARD(float spentamt){float point=0;
      while(spentamt>=200){point+=25;spentamt-=200;}return point;}
}
...
public static class Company
{
    ...
    public void setdelivery(float d){delivery+=d;}
    public float getamount(){return amount;}
    public float getdelivery(){return delivery;}
}
public static void setAmountforCompany(float a,Company C){C.setamount(a*(float)0.01);}
public static void setDeliveryforCompany(float a,Company C){C.setdelivery(a);}
...
public class CHOICE1 implements CHOICE
{
  ...
  System.out.println("2) Ravi");
  System.out.println("3) The Chinese (Authentic)");
  System.out.println("4) Wang (Fast Food)");
  ...
  choice2=in.nextInt();
    ...
    choice3=in.nextInt();
    if(choice3==1){RList.get(choice2-1).Addfood();}
    if(choice3==2){RList.get(choice2-1).Editfood();}
    if(choice3==3){RList.get(choice2-1).PrintReward();}
    if(choice3==4){RList.get(choice2-1).setprdis();}
  }while(choice3!=5);
}}
}
...
public class CHOICE2 implements CHOICE
{
  ...
  int choice3=0;
    ...
    if(choice2!=6){
      ...
      for(int i=0;i<RList.get(Rchoice-1).getFoodList().size();i++){System.out.println(RList.get(Rchoice-1).getFoodList().get(i));}
      ...
      if(choice3==2){CList.get(choice2-1).Checkout(CList,RList,choice2-1,C,0);}
      if(choice3==3){CList.get(choice2-1).PrintReward();}
      if(choice3==4){CList.get(choice2-1).Display(CList,RList,choice2-1);}
    }while(choice3!=5);}
}
}
  ...
  public void Choice(ArrayList<Customer> CList,ArrayList<Restaurant> RList,Company C){
    ...
    if(choice2==1){
      for(int i=0;i<CList.size();i++){
          ...
          choice3=in.nextInt();
          System.out.println(CList.get(choice3-1).getname()+" "+CList.get(choice3-1).getCategory()+" "+Float.toString(CList.get(choice3-1).getamount()));
      }
    else{
      for(int i=0;i<RList.size();i++){
          System.out.println(Integer.toString(i+1)+".  "+RList.get(i).getname());}
          choice3=in.nextInt();
          System.out.println(RList.get(choice3-1).getname()+" "+Integer.toString(RList.get(choice3-1).getnorder())+" order(s)");
      }
}
}
...
public static class Menu
{
  ...
  APA2 A=new APA2();
   ...
   Elite Ram=new Elite("Ram");
   Elite Sam=new Elite("Sam");
   Special Tim=new Special("Tim");
   Customer Kim=new Customer("Kim");
   Customer Jim=new Customer("Jim");

   Authentic Shah=new Authentic("Shah");
   Restaurant Ravi=new Restaurant("Ravi");
   Authentic Chinese=new Authentic("The Chinese");
   FastFood Wang=new FastFood("Wang");
   ...
   Menu(){
     ...
     do{
       ...
       choice1=in.nextInt();
       switch(choice1){
         case 1:CHOICE choice1=A.new CHOICE1();
         useChoice(choice1,CList,RList,C);
         break;
         case 2:CHOICE choice2=A.new CHOICE2();
         useChoice(choice2,CList,RList,C);
         break;
         case 3:CHOICE choice3=A.new CHOICE3();
         useChoice(choice3,CList,RList,C);
         break;
         case 4:System.out.println("Total Company Balance: "+Float.toString(C.getamount()));
